...
var findLadders = function(beginWord, endWord, wordList) {
    ...
    // avoid transform to the beginWord in the middle
    ...
    while(queue.length > 0) {
        ...
        if (begin == endWord) {
            // record the shortest len to only output the shortest path
            //console.log(shortest);
            if (shortest === undefined) {
                shortest = data.len;
                ladders.push(data.ladder);
            } else if (data.len == shortest) {
                ladders.push(data.ladder);
            } else if (data.len > shortest) {
                return ladders;
            }
        ...
        //console.log(data);
        var neighbors = findNeighbors(begin, endWord, wordList, data.ladder);
        neighbors.forEach(function (key, value) {
            var ladder = [];
        ...
        wordList.delete(begin);
    }

    return ladders;
};

var findNeighbors = function (begin, end, dic, visited) {
    var arr = begin.split('');
    var temp = '';
    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        for (var j = 0; j <= 25; j++) {
            ...
            if (dic.has(newBegin) && visited.indexOf(newBegin) == -1) {
...
//var set = new Set(arr);
//console.log(findLadders('hit', 'cog', set));

// test case 2: found my code will find all transform instead of the shortest one
//var arr = ["a","b","c"];
//var set = new Set(arr);
//console.log(findLadders('a', 'c', set));

// test case 3: found my code error with ascii
//var arr = ["ted","tex","red","tax","tad","den","rex","pee"];
//var set = new Set(arr);
//console.log(findLadders('red', 'tax', set).length);

// test case 4: time limit exceed
//var arr = ["si","go","se","cm","so","ph","mt","db","mb","sb","kr","ln","tm","le","av","sm","ar","ci","ca","br","ti","ba","to","ra","fa","yo","ow","sn","ya","cr","po","fe","ho","ma","re","or","rn","au","ur","rh","sr","tc","lt","lo","as","fr","nb","yb","if","pb","ge","th","pm","rb","sh","co","ga","li","ha","hz","no","bi","di","hi","qa","pi","os","uh","wm","an","me","mo","na","la","st","er","sc","ne","mn","mi","am","ex","pt","io","be","fm","ta","tb","ni","mr","pa","he","lr","sq","ye"];
//var set = new Set(arr);
//console.log(findLadders('qa', 'sq', set).length);

// test case 5: time limit exceed
...
var set = new Set(arr);
console.log(findLadders('sand', 'acne', set).length);
